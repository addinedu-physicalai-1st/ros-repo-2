...
YOLO_CONF = float(os.environ.get('YOLO_CONFIDENCE', '0.75')) # Hardened 임계값
...
def infer(model: YOLO, reid: ReIDEngine, frame_bytes: bytes, confidence: float = 0.25) -> list[dict]:
    ...
    # ByteTracker 적용 (persist=True) - retina_masks=True 로 고해상도 마스크 획득
    results = model.track(img, persist=True, conf=confidence, verbose=False, retina_masks=True, imgsz=640)
    ...
    try:
        with conn:
            while True:
                ...
                result = infer(current_model, reid, frame, confidence=current_conf)
