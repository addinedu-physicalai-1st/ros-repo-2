...
if ! command -v tmux &>/dev/null; then
    ...
    echo ""
    echo "  [1] control_service : $TMUX_SRC && $ROS_ENV && cd $ROS_WS && python3 $ROS_WS/install/control_service/lib/control_service/main"
    [ "$NO_AI" = false ] && \
...
# ros2 run 은 shebang(/usr/bin/python3) 을 쓰므로 conda pip 가 무시됨 → python3 직접 호출
# macOS SIP 가 /usr/bin/env 경유 시 DYLD_LIBRARY_PATH 를 제거하므로 env 사용 금지
tmux send-keys -t "${SESSION}:control" \
    "$TMUX_SRC && $ROS_ENV && cd $ROS_WS && python3 $ROS_WS/install/control_service/lib/control_service/main" Enter
