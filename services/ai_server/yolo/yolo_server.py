...
CONFIG_PATH = os.path.join(os.path.dirname(__file__), 'active_model.json')
...
def infer(model: YOLO, reid: ReIDEngine, frame_bytes: bytes, confidence: float = 0.25) -> list[dict]:
    ...
    for r in results:
        ...
        for i in range(len(r)):
            ...
            # Segmentation 마스크 추출 (있는 경우)
            mask_pts = None
            if hasattr(r, 'masks') and r.masks is not None:
                # r[i] 에서 마스크 xy 리스트 추출
                m = r.masks.xy[i]
                if len(m) > 0:
                    mask_pts = m.tolist()
    ...
    try:
        with conn:
            while True:
                ...
                # 추론 (YOLO + ReID)
                result = infer(model_manager.get_model(), reid, frame)
