...
| 110 | 입구 | 하단 좌측 | 미정 |
| 120 | 출구 | 하단 우측 | 미정 |
| 140 | 카트 충전소 주차 1번 (P1) | 하단 중앙, 병렬 주차 슬롯 1. 북쪽(theta=90°) 방향 | 미정 |
| 141 | 카트 충전소 주차 2번 (P2) | 하단 중앙, 병렬 주차 슬롯 2. 북쪽(theta=90°) 방향 | 미정 |
| 150 | 결제 구역 | 우측 하단 — 자동 결제 트리거 구역 | 미정 |

> **카트 충전소 배치:** 하단 중앙에 위치하며 카트들은 북쪽(맵 위 방향, theta=90°)을 바라보고 병렬로 주차된다. 슬롯은 P1(140), P2(141) 2개이며, 로봇 귀환 시 `/zone/parking/available`로 빈 슬롯을 조회해 직접 해당 슬롯으로 이동한다.
