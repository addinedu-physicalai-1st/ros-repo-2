...
tmux send-keys -t "${SESSION}:nav" \
    "$TMUX_SRC && $ROS_ENV && export ROBOT_ID=$ROBOT_ID && export CONTROL_SERVICE_HOST=$CUSTOMER_WEB_HOST && cd $ROS_WS && sleep 10 && ros2 launch shoppinkki_nav navigation.launch.py" Enter

# 창 2: shoppinkki_core (SM + BT + HW)
# Nav2 초기화 대기 후 실행 (30초)
tmux new-window -t "${SESSION}" -n "core"
tmux send-keys -t "${SESSION}:core" \
    "$TMUX_SRC && $ROS_ENV && export ROBOT_ID=$ROBOT_ID && export CUSTOMER_WEB_HOST=$CUSTOMER_WEB_HOST && export CONTROL_SERVICE_HOST=$CUSTOMER_WEB_HOST && export YOLO_HOST=$CUSTOMER_WEB_HOST && sleep 30 && ros2 run shoppinkki_core main_node" Enter
