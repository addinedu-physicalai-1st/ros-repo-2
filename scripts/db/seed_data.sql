...
ON CONFLICT (product_name) DO UPDATE SET zone_id = EXCLUDED.zone_id;
=======
('TV',       1, 990000), ('냉장고',   1, 1290000), ('에어컨',   1, 1590000),
('쌀과자',   2,   2000), ('포카칩',   2,    1800), ('오레오',   2,    2500),
('연어',     3,  12000), ('새우',     3,    9000), ('오징어',   3,    8000),
...
>>>>>>> 457f0d3 (feat: improve search accuracy and fix container networking)
...
FROM (
  VALUES
<<<<<<< HEAD
  ('TV', '큰 화면으로 드라마, 영화, 방송 같은 영상 콘텐츠를 시청하는 전자기기이다.'),
  ('냉장고', '음식과 음료를 차갑게 보관해 신선도를 유지하는 가전제품이다.'),
  ('에어컨', '실내를 시원하게 유지하고 냉방이나 제습에 사용하는 가전제품이다.'),
  ('쌀과자', '쌀을 원료로 만든 과자로 담백하고 바삭한 간식에 해당한다.'),
  ('포카칩', '감자를 얇게 썰어 만든 과자로 짭짤하고 바삭한 감자칩 간식이다.'),
  ('오레오', '초코 쿠키와 크림으로 구성된 과자로 달콤한 디저트 간식에 해당한다.'),
  ('연어', '부드럽고 고소한 생선류 식품으로 회, 샐러드, 구이 재료로 사용한다.'),
  ('새우', '탱글한 식감의 갑각류 식품으로 볶음, 튀김, 파스타 재료로 사용한다.'),
  ('오징어', '쫄깃한 식감의 연체류 식품으로 볶음, 구이, 안주 재료로 사용한다.'),
  ('소고기', '풍미가 진한 육류로 구이, 스테이크, 국거리 재료로 사용한다.'),
  ('돼지고기', '고소한 맛의 육류로 삼겹살, 구이, 볶음 요리에 사용한다.'),
  ('닭고기', '담백한 육류로 구이, 볶음, 샐러드 재료로 사용한다.'),
  ('당근', '아삭한 뿌리채소로 샐러드, 볶음, 주스 재료로 사용한다.'),
  ('브로콜리', '식감이 단단한 채소로 데침, 볶음, 샐러드 재료로 사용한다.'),
  ('상추', '신선한 잎채소로 쌈이나 샐러드에 사용하는 채소이다.'),
  ('콜라', '차갑게 마시는 탄산음료로 단맛과 탄산감이 있는 음료이다.'),
  ('커피', '원두를 추출해 만드는 음료로 따뜻하게 또는 차갑게 마시며 카페인이 포함된다.'),
  ('오렌지주스', '오렌지 과즙으로 만든 음료로 상큼한 과일 맛이 나는 주스이다.'),
  ('식빵', '밀가루 반죽을 구워 만든 빵으로 토스트나 샌드위치에 사용한다.'),
  ('크루아상', '버터를 넣은 페이스트리 빵으로 결이 겹겹이 나고 바삭한 식감이 있다.'),
  ('머핀', '작은 케이크 형태의 빵으로 달콤한 간식이나 디저트로 먹는다.'),
  ('케이크', '생일이나 기념일, 파티를 축하하기 위해 먹는 달콤하고 예쁜 케이크이다.'),
  ('볶음밥', '밥과 재료를 함께 볶아 만드는 음식으로 든든한 한 끼 식사에 해당한다.'),
  ('라면', '면과 스프를 끓여 만드는 음식으로 뜨거운 국물과 매운맛을 포함할 수 있다.'),
  ('떡볶이', '떡을 양념 소스에 조리한 음식으로 매콤달콤한 분식 메뉴에 해당한다.')
=======
