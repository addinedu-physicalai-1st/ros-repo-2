...
### 2. 🗺️ Fleet 라우팅 아키텍처 (자체 구현)
* **Open-RMF를 사용하지 않습니다.** 다중 로봇 조정은 `server/control_service/control_service/fleet_router.py`의 그래프 라우팅 + 레인 예약으로 구현합니다.
* **웨이포인트 소스:** `server/control_service/map/shop_nav_graph.yaml` (39 vertices + lanes)을 DB `fleet_waypoint` / `fleet_lane` 테이블로 시딩하여 사용합니다.
* **경로 실행:** Nav2 (`navigate_to_pose` / `navigate_through_poses`)가 담당합니다. `fleet_router`는 그래프상 vertex 순서만 계산하고, 각 구간 실행은 Nav2에 위임합니다.
* **메시지 타입:** 표준 `geometry_msgs`, `nav_msgs`, `std_msgs`만 사용합니다. `rmf_*` 메시지는 사용하지 않습니다.
...
* **QoS (Quality of Service):** 센서 데이터(IMU, LiDAR 등)는 `SensorDataQoS`, 제어 및 상태 메시지는 `Reliable` 정책을 명시적으로 설정하여 통신 유실을 방지하세요.
...
### 5. 🗺️ Nav2 및 SLAM 가이드라인 (Nav2 & SLAM Toolbox)
...
├── server/                  ← 서버 PC 실행
│   ├── control_service/             ← ROS2 노드 + TCP(8080) + REST API(:8081) + fleet_router
